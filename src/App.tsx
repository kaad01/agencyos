...
import { addDays, byId, calculateMetrics, colleagueBillableRatio, colleagueDeliveryLoadPercent, colleagueLoadStatus, colleagueLoggedHours, colleagueOpenTicketEstimate, customerHours, customerProjects, customerReportRollups, customerRevenue, customerTickets, formatCurrency, initialData, makeId, moveTicketOnBoard, priorities, projectBillableHours, projectBudgetRemaining, projectBudgetUsedPercent, projectEffectiveRate, projectEstimateUsedPercent, projectEstimatedHours, projectHours, projectNonBillableHours, projectRemainingEstimateHours, projectRevenue, projectDeliverySignal, filterTimeEntriesForReport, filterTimeEntriesForTimesheet, projectStatuses, reportScopeInsights, roundedTimerHours, ticketDeliverySignal, ticketEstimateUsedPercent, ticketLoggedHours, ticketStatuses, weekStartDate, weeklyTimesheetByColleague, weeklyTimesheetByCustomer, weeklyTimesheetByProject, timeEntryDraftForTicket, timeEntryDraftForTimesheetScope, timeEntryImpactPreview, weeklyTimeCaptureFocus, weeklyTimerStarterQueue, weeklyTimesheetApprovalSnapshot, weeklyTimesheetAudit, weeklyTimesheetCapacity, weeklyTimesheetClientPacket, weeklyTimesheetCoach, weeklyTimesheetDayHealth, weeklyTimesheetDayReviewBriefs, weeklyTimesheetDayStartSuggestions, weeklyTimesheetReview, weeklyTimesheetReviewQueue, weeklyTimesheetValueSummary, weeklyUnloggedTickets, type AppData, type Colleague, type Customer, type CustomerHealth, type Project, type ProjectStatus, type Ticket, type TicketPriority, type TicketStatus, type ReportPeriod, type ReportBillableFilter, type TimeEntry, type TimeEntryDraft } from './domain';
...
const reportBillableFilters: Record<ReportBillableFilter, string> = { all: 'All work', billable: 'Billable only', internal: 'Internal only' };
  ...
  return (
    <main className="shell">
      ...
      <section className="workspace">
        ...
        {activeTimer && <ActiveTimerBar data={data} activeTimer={activeTimer} now={now} onStop={stopTimer} onReview={reviewTimerEntry} onCancel={cancelTimer} />}
        ...
        {view === 'Time' && <TimeView data={data} activeTimer={activeTimer} now={now} onStartTimer={startTimer} onStopTimer={stopTimer} onReviewTimer={reviewTimerEntry} onCancelTimer={cancelTimer} onNew={openTimeEntryDraft} onQuickLog={quickLogTicket} onEdit={editTimeEntry} onDelete={deleteTimeEntry} onExport={exportCsv} />}
...
function TimeView({ data, activeTimer, now, onStartTimer, onStopTimer, onReviewTimer, onCancelTimer, onNew, onQuickLog, onEdit, onDelete, onExport }: { data: AppData; activeTimer: ActiveTimer | null; now: number; onStartTimer: (projectId: string, ticketId?: string, options?: TimerStartOptions) => void; onStopTimer: () => void; onReviewTimer: () => void; onCancelTimer: () => void; onNew: (draft?: TimeEntryDraft | null) => void; onQuickLog: (ticketId: string, date: string) => void; onEdit: (entryId: string) => void; onDelete: (entryId: string) => void; onExport: (entries?: TimeEntry[]) => void }) {
  ...
  const approvalSnapshot = weeklyTimesheetApprovalSnapshot(data, { weekDate: selectedWeekDate, projectId, colleagueId });
  const valueSummary = weeklyTimesheetValueSummary(data, { weekDate: selectedWeekDate, projectId, colleagueId });
  const clientPacket = weeklyTimesheetClientPacket(data, { weekDate: selectedWeekDate, projectId, colleagueId });
  const coach = weeklyTimesheetCoach(data, { weekDate: selectedWeekDate, projectId, colleagueId });
  const dayHealth = weeklyTimesheetDayHealth(data, { weekDate: selectedWeekDate, projectId, colleagueId });
  const dayStartSuggestions = new Map(weeklyTimesheetDayStartSuggestions(data, { weekDate: selectedWeekDate, projectId, colleagueId }).map((day) => [day.date, day.suggestion]));
  const dayReviewBriefs = weeklyTimesheetDayReviewBriefs(data, { weekDate: selectedWeekDate, projectId, colleagueId });
  const captureFocus = weeklyTimeCaptureFocus(data, { weekDate: selectedWeekDate, projectId, colleagueId });
  const timerQueue = weeklyTimerStarterQueue(data, { weekDate: selectedWeekDate, projectId, colleagueId }).slice(0, 3);
...
function TimerSavePreview({ data, activeTimer, now, compact = false }: { data: AppData; activeTimer: ActiveTimer; now: number; compact?: boolean }) {
  ...
  const impact = timeEntryImpactPreview(data, {
    projectId: activeTimer.projectId,
    ticketId: activeTimer.ticketId,
    colleagueId: activeTimer.colleagueId,
    date: todayPlus(0),
  ...
  return <div className={compact ? 'timerSavePreview compact' : 'timerSavePreview'}><span><strong>{impact.hours}h</strong><small>will be saved</small></span><span><strong>{activeTimer.billable ? formatCurrency(impact.revenue) : 'Internal'}</strong><small>{activeTimer.billable ? 'projected revenue' : 'non-billable work'}</small></span><span><strong>{impact.estimateUsedAfter}% estimate</strong><small>{impact.deliverySignalAfter} after save</small></span><span><strong>{colleagueName(data, activeTimer.colleagueId)}</strong><small>{activeTimer.note}</small></span></div>;
}

function TimerStartPreview({ data, projectId, ticketId, colleagueId, billable, note }: { data: AppData; projectId: string; ticketId: string; colleagueId: string; billable: boolean; note: string }) {
  if (!projectId || !colleagueId) return null;
  const impact = timeEntryImpactPreview(data, {
    projectId,
    ticketId,
    colleagueId,
  ...
  return <div className="timerStartPreview" aria-label="Timer start impact preview"><span><strong>{projectName(data, projectId)}</strong><small>{ticketId ? ticketTitle(data, ticketId) : impact.needsTicket ? 'No ticket linked yet' : 'Project-level time'}</small></span><span><strong>{billable ? formatCurrency(impact.revenue) : 'Internal'}</strong><small>first 15 min impact</small></span><span><strong>{impact.estimateUsedAfter}% estimate</strong><small>{impact.ticketEstimateUsedAfter === null ? `${impact.budgetUsedAfter}% budget after first save` : `${impact.ticketEstimateUsedAfter}% ticket estimate`}</small></span></div>;
}

function TimerControl({ data, activeTimer, now, onStartTimer, onStopTimer, onReviewTimer, onCancelTimer }: { data: AppData; activeTimer: ActiveTimer | null; now: number; onStartTimer: (projectId: string, ticketId?: string, options?: TimerStartOptions) => void; onStopTimer: () => void; onReviewTimer: () => void; onCancelTimer: () => void }) {
...
function ReportsView({ data, metrics, onExport, onLogTime }: { data: AppData; metrics: ReturnType<typeof calculateMetrics>; onExport: (entries?: TimeEntry[]) => void; onLogTime: () => void }) {
  ...
  const [projectId, setProjectId] = useState('');
  const [colleagueId, setColleagueId] = useState('');
  const [billable, setBillable] = useState<ReportBillableFilter>('all');
  ...
  const reportTimeEntries = filterTimeEntriesForReport(data, { period, customerId, projectId: selectedProjectId, colleagueId, billable });
  ...
  const reportInsights = reportScopeInsights(data, reportTimeEntries);
...
function TimeRow({ data, entry, onEdit, onDelete, onResume, resumeDisabled = false }: { data: AppData; entry: TimeEntry; onEdit?: (entryId: string) => void; onDelete?: (entryId: string) => void; onResume?: (entry: TimeEntry) => void; resumeDisabled?: boolean }) { return <div className="timeRow"><TimerReset size={18}/><div><strong>{entry.hours}h · {projectName(data, entry.projectId)}</strong><small>{ticketTitle(data, entry.ticketId)} · {colleagueName(data, entry.colleagueId)} · {entry.date}</small>{entry.note ? <small>{entry.note}</small> : null}</div><em className={entry.billable ? 'green' : 'neutral'}>{entry.billable ? 'Billable' : 'Internal'}</em>{(onEdit || onDelete || onResume) && <div className="rowActions" aria-label={`Time entry actions for ${projectName(data, entry.projectId)}`}>{onResume && <button type="button" disabled={resumeDisabled} title={resumeDisabled ? 'Stop or discard the running timer before resuming another entry.' : undefined} onClick={() => onResume(entry)}>Resume</button>}{onEdit && <button type="button" onClick={() => onEdit(entry.id)}>Edit</button>}{onDelete && <button type="button" className="danger" onClick={() => onDelete(entry.id)}>Delete</button>}</div>}</div>; }
