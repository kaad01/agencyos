...
export function App() {
  ...
  return (
    <main className="shell">
      ...
      {modal && <EntityModal modal={modal} data={data} selectedProjectId={selectedProject?.id ?? ''} editingTicket={editingTicketId ? byId(data.tickets, editingTicketId) : undefined} editingTimeEntry={editingTimeEntryId ? byId(data.timeEntries, editingTimeEntryId) : undefined} onClose={() => { setModal(null); setEditingTicketId(null); setEditingTimeEntryId(null); }} onProject={addProject} onTicket={addTicket} onTime={saveTimeEntry} onCustomer={addCustomer} onColleague={addColleague} />}
    </main>
  );
}
...
function EntityModal({ modal, data, selectedProjectId, editingTicket, editingTimeEntry, onClose, onProject, onTicket, onTime, onCustomer, onColleague }: { modal: Modal; data: AppData; selectedProjectId: string; editingTicket?: Ticket; editingTimeEntry?: TimeEntry; onClose: () => void; onProject: (form: FormData) => void; onTicket: (form: FormData) => void; onTime: (form: FormData) => void; onCustomer: (form: FormData) => void; onColleague: (form: FormData) => void }) {
